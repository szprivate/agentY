# Seedance 2.5 prompt optimizer — vendored reference

> **Source.** ByteDance's own `sd25-pe` skill (v0.1.1, `owner: seedance`), the one the
> Seedance 2.5 prompting docs tell you to install with
> `npx skills@latest add "https://arkdocs-en.tos-ap-southeast-1.volces.com/skills/" --skill sd25-pe`.
> Retrieved 2026-08-13. **No licence, copyright, or terms accompanied the file.**
>
> **Changes from the original.** The body below is verbatim except for two things:
> its YAML frontmatter (a skill manifest, meaningless as a reference file) and its
> `## Self-update before triggering` section (17 lines), which instructed the
> agent to run `npx --yes skills@latest update sd25-pe -y` on first use in every
> session. agentY does not let a prompt rewrite itself from the network mid-run:
> what is in this file is what the model reads. Re-vendor deliberately instead.
>
> **Read this with `seedance.md` open.** Its parameter rules describe ByteDance's
> ModelArk REST API (roles, `duration: -1`, `mov` output). agentY drives ComfyUI's
> `ByteDance2*` nodes, whose contract differs — `seedance.md` and the
> `seedance-reference` skill carry the translation. Where the two disagree about a
> **parameter**, the node wins. Where they disagree about **prompt craft**, this
> file wins: it is the model owner's own guidance.
