...
def _get_triage_config() -> tuple[str, str]:
    """Return ``(triage_model, host)`` for intent classification from ``settings.json``.

    Reads ``llm.pipeline.triage``; falls back to ``qwen3:0.6b`` when not set.
    """
    settings = _load_settings()
    llm = settings.get("llm", {})
...
class LLMFunctions:
    ...
    def for_triage(cls) -> "LLMFunctions":
        """Construct a small, fast instance for triage (``llm.pipeline.triage`` key)."""
