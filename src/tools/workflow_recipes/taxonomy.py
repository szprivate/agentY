...
With ``--cluster-on category`` (the default), workflows are grouped by the
category returned here, so each "type" in the database is one canonical category.
"""
...
def _input_modalities(graph: WorkflowGraph) -> Set[str]:
    """Content modalities entering the workflow, from boundary ports (collapsed
    subgraphs) plus load nodes (flat graphs)."""
...
def _task_category(graph: WorkflowGraph, gen: bool) -> str:
    """The task category from output media + input modality + name. ``gen`` is
    whether the workflow can *generate* content (a local diffusion sampler, or a
    partner generation node) - used so a non-diffusion partner video graph is not
...
def classify(graph: WorkflowGraph) -> str:
    classes = {n.class_type.lower() for n in graph.nodes.values()}
    name = graph.name.lower()
    cat = graph.category
    ...
    if any(n.is_api for n in graph.nodes.values()) or name.startswith("api_"):
        return "API / Partner Nodes - " + _task_category(graph, gen=True)
    ...
    return _task_category(graph, gen=diffusion)
