...
class PrompterBase:
    """Base class encapsulating common LLM functionality for producer, writer
    and supervisor.  The old ``prompter`` class was split into this base plus
    specialized subclasses in their own modules.
    """
    ...
    def __init__(
        ...
        prompt_guide_default = prompt_guide_default or Path(
            SYS_CONFIG["prompts"]["guide_keyframes"]
        )
        prompt_persona_supe = prompt_persona_supe or Path(
            SYS_CONFIG["prompts"]["persona_supervisor"]
        )
        prompt_persona_writer = prompt_persona_writer or Path(
            SYS_CONFIG["prompts"]["persona_writer"]
        )
        prompt_persona_producer = prompt_persona_producer or Path(
            SYS_CONFIG["prompts"]["persona_producer"]
        )

        self.api_base_url = api_base_url.replace("/v1", "")
    ...
    def _load_llm_prompt(self, prompt_file: Path) -> str:
    ...
    def _encode_and_resize_image(
        self, image_path: Path, scale_factor: float = 0.5
    ) -> str:
    ...
    def _encode_existing_images(
        self, image_paths: Optional[List[Path]]
    ) -> List[str]:
...
class Generator:
    ...
    def __init__(
        ...
        comfyui_output_dir = comfyui_output_dir or (
            Path(SYS_CONFIG.get("comfyui_output_dir_override"))
            if SYS_CONFIG.get("comfyui_output_dir_override")
            else Path(default_out)
        )
    ...
    def generate(
        ...
        positive_prompt: str,
        negative_prompt: str,
        mood_images: Optional[List[Path]] = None,
