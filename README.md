...
- **Fill a slot nothing is wired to** — an unwired input is *absent* from the graph, so ten empty reference slots look exactly like a node that has none. The agent reads what the node really declares from ComfyUI's schema, adds a loader and connects it — for that run only, leaving your canvas untouched. That is what makes *“run those two again, but with this photo as a reference”* work. See [Filling a slot](docs/using-agentY.md#filling-a-slot-nothing-is-wired-to).
- **Screenshots of your canvas** — *"send me a screenshot of my workflow on Slack"*. The picture is drawn by the ComfyUI page itself, so it is your graph as **you** have it — your layout, groups, colours and collapsed nodes — and your view is put back before the browser paints, so nothing moves on screen. Prompts and hook directives are HTML boxes floating above the canvas, invisible to a canvas capture, so agentY paints them back in at the size and position the real box has — the picture has the text a screenshot would. A graph too large to draw at readable zoom comes back as a labelled overview and the agent says so; select the part you care about and it is photographed at full size. The agent is also told when several workflows are open in ComfyUI's tabs, and that only the active one is live. See [Screenshots](docs/using-agentY.md#screenshots-of-your-workflow).
